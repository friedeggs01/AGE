...
class AGEUtils:
    ...
    def AGESelection(self, population):
        ...
        d = 0
        while (len(new_population.population) + len(population.fronts[d])) < len(population.population):
            population.count_survival_score(d)
            new_population.extend(population.fronts[d])
            d += 1
        population.count_survival_score(d)
        population.fronts[d].sort(key=lambda ind: ind.survival_score, reverse=True)
        new_population.extend(population.fronts[d][0: (len(population) - len(new_population))])
